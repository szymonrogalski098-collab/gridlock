...
let _cubek2ThenHard = false;    // [2.0-s2] hard flag stashed while Cubek 2.0 plays
...
function applyBoardSkin() {
  ...
  if (tutBoardEl) {
    tutBoardEl.style.background = boardBg;
    tutBoardEl.style.setProperty('--board-bg', cellBg);
  }
}
...
function drawBoardGridLines(ctx2, canvasSize, n) {
  ...
  if (w2) return; // [2.0-s2] World 2 is a seamless dark void — no grid lines
  if (boardSkinId === 'classic' || !showBoardGrid) return; // [1.9.1] bug #8: no-grid toggle
  const skin = BOARD_SKINS[boardSkinId] || BOARD_SKINS.classic;
  const gridColor = skin.grid;
  const useGlow   = skin.glow;
...
function render() {
  ...
  if (!bossRound) blocks = blocks.filter(b => !b.bossRain && !b.bossThrow); // [2.0-s3] never show boss blocks outside boss rounds
...
function drawAsteroids(now) {
  ...
  for (const a of asteroids) {
    ...
    if (cx>=0&&cx<N&&cy>=0&&cy<N && cx===cube.x && cy===cube.y
        && !blackHoleAnimating && !(testerActive && tNoclip) && !customGame) { // [2.0-s3.2] immortal in sandbox
      die('asteroid'); return;
    }
  }
}
...
function _bhFinish() {
  ...
  blackHoleAnimating = false; blackHole = null;
  if (!alive) return;
  blackHoleCooldown = customGame ? 0 : 3; // [2.0-s3.3] unlimited in the sandbox (no rounds to tick it down)
  render();
}
...
function tryDash(x,y) {
  ...
  if (dashesLeft<=0 && !(testerActive && tDashInf) && !bossRound && !customGame){flash('No dash available!');return;} // [1.9][1.11][2.0-s3.3]
  ...
  playSound('dash');
  if (!bossRound && !customGame) dashesLeft--; // [1.11][2.0-s3.3] unlimited dashes during boss / sandbox
...
const ROUND_MODS = [
  ...
  { id:'grid_glitch',  name:'GRID GLITCH',  hint:'glitch tear · ×1.5 reward', category:'COSMETIC', weight:2,
    onStart(){ roundCoinMult = 1.5; if (!boardTear) _setBoardTear(); document.getElementById('board-wrap')?.classList.add('board-tear'); }, // [2.0-s3.2]
    onEnd(){ _clearBoardTear(); } },
];

// [2.0-s3.2] Grid Glitch — diagonal board tear between two random opposite-edge cells (cosmetic only)
function _setBoardTear() {
  const wrap = document.getElementById('board-wrap');
  if (!wrap) return;
  const horizontal = Math.random() < 0.5;
  const p1 = ((Math.floor(Math.random()*N) + 0.5) / N * 100).toFixed(1); // entry cell %
  const p2 = ((Math.floor(Math.random()*N) + 0.5) / N * 100).toFixed(1); // opposite-edge cell %
  let aPoly, bPoly, ox, oy;
  if (horizontal) { // tear runs left→right at a shallow angle; halves = top / bottom
    aPoly = `polygon(0 0, 100% 0, 100% ${p2}%, 0 ${p1}%)`;
    bPoly = `polygon(0 ${p1}%, 100% ${p2}%, 100% 100%, 0 100%)`;
    ox = '3px'; oy = '8px'; // [2.0-s3.3] bigger offset = obvious split
  } else {           // tear runs top→bottom; halves = left / right
    aPoly = `polygon(0 0, ${p1}% 0, ${p2}% 100%, 0 100%)`;
    bPoly = `polygon(${p1}% 0, 100% 0, 100% 100%, ${p2}% 100%)`;
    ox = '8px'; oy = '3px'; // [2.0-s3.3]
  }
  wrap.style.setProperty('--tear-a', aPoly);
  wrap.style.setProperty('--tear-b', bPoly);
  wrap.style.setProperty('--tear-ox', ox);
  wrap.style.setProperty('--tear-oy', oy);
  boardTear = { horizontal };
}
function _clearBoardTear() { // [2.0-s3.2]
  boardTear = null;
  const wrap = document.getElementById('board-wrap');
  if (wrap) {
    wrap.classList.remove('board-tear');
...
let cubek2Step = 0;
function showCubek2() { // [2.0-s1] first-time-only cosmic intro
...
function cubek2Next() { // [2.0-s1]
  ...
  if (++cubek2Step >= CUBEK2_LINES.length) {
    localStorage.setItem('cm_cubek2_done', 'true');
    startGame(_cubek2ThenHard); // [2.0-s2] honor requested difficulty after the intro
    _cubek2ThenHard = false;
  } else {
    renderCubek2();
  }
}
...
function beginGame(hard) {
  if (currentWorld === 2 && localStorage.getItem('cm_cubek2_done') !== 'true') {
    _cubek2ThenHard = hard; showCubek2(); return;
  }
  startGame(hard);
}
...
function startRound() {
  ...
  if (BOSS_ROUNDS.includes(round)) { // [1.11][2.0-s2] bosses in all modes incl. Time Attack (both worlds)
...
function drawSolarFlares(now) {
  const fireFlares   = lasers.filter(l => l.state === 'fire');
  const chargeFlares = lasers.filter(l => l.state === 'charge');
  const pulse = .45 + .35*Math.sin(now*.009);

  // Warning (charge) — semi-transparent warm band, same feel as the World 1 laser warning
  ...
  // Beam (fire) — fully opaque bright orange + hot yellow glow
  if (fireFlares.length > 0) {
    ...
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.globalCompositeOperation = 'lighter';
    ctx.fillStyle = 'rgba(255,215,70,.6)';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.globalCompositeOperation = 'source-over';
    ctx.restore();
  }
...
const SCREENS = ['screen-start','screen-stats','screen-pin','screen-missions','screen-tutorial','screen-modes','screen-world-choice','screen-cubek2','app']; // [1.10] [1.10.1] removed screen-tester [2.0-s1] world screens
...
function startGame(hard = false, fromTester = false, custom = false) {
  hardMode = hard;
  ...
  blackHoleCooldown=0; _resetBlackHole(); // [2.0-s2]
  if (_asteroidsEnabled()) scheduleAsteroid(); // [2.0-s2][2.0-s3.1] arm asteroid spawner
  ...
  buildBoard(); render();
  if (customGame) _customStart(); else startRound(); // [2.0-s3.2] sandbox vs normal loop
  mTrackGameStart();
...
const TUT_STEPS = [ // [1.9] all text translated to English
  ...
  },
  {
    id:'hard_mode',
    face:'😏',
    text:'For the brave: <span class="tut-warn">🔥 Hard Mode</span>.<br>Lasers 1.6× faster, but you earn <strong style="color:#ffd700">3× more coins</strong>!<br>Master normal mode first.', // [2.0-s2] fixed multiplier
    hint:'',
    next:'I\'m ready!',
    action:null
  }
...
document.getElementById('wc-world2').addEventListener('click', () => { // Enter the Void
  ...
  applyWorldTheme();
  _cubek2ThenHard = false; // [2.0-s2] entering the Void fresh is always Normal
