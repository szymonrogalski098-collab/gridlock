...
function playSound(type) {
  try {
    ...
    if (type === 'dash') {
      ...
      o.connect(f); f.connect(g); o.start(); o.stop(ac.currentTime + 0.5);
      ...
      src.connect(f); f.connect(g); src.start(); src.stop(ac.currentTime + 0.12);
      ...
      o.connect(g); o.start(); o.stop(ac.currentTime + 0.3);
      ...
      g.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + 0.5);
      src.connect(f); f.connect(g); src.start(); src.stop(ac.currentTime + 0.5);
      ...
      o.connect(g); o.start(); o.stop(ac.currentTime + 0.1);
    } else if (type === 'click') { // [1.9.1] UI click sound for tutorial NEXT button
      const o = ac.createOscillator();
      o.type = 'sine';
      ...
      o.connect(g); o.start(); o.stop(ac.currentTime + 0.08);
    }
  } catch(e) { /* audio may be blocked on some devices */ } // [1.9.1] translated
}
...
const _ph = 'bbc9488202c05bf93f12acb5ad207cf2c5da040f7781c1a48513b30afceb69c0';
...
function updatePinDisplay() {
  const shown = pinBuffer.padEnd(4,'_').split('').map((c,i)=>i<pinBuffer.length?'●':'_').join(' ');
...
function startGame(hard = false, fromTester = false) {
  ...
  // odblokuj skiny tylko z testera
  if (fromTester) {
...
function buySkin(id){
  ...
  if (owned.includes(id)){skinId=id; invalidateSkinCache(); save();renderShop();return;}
  if (coins<s.price){shopBal.textContent=`Not enough 🪙 (you have ${coins}, need ${s.price})`;return;} // [1.9]
  coins-=s.price; owned.push(id); skinId=id; invalidateSkinCache(); save(); renderShop();
}
...
function buyBoardSkin(id) { // [1.9]
  ...
  if (boardsOwned.includes(id)) {
    ...
    applyBoardSkin(); renderShop(); return;
  }
  if (def.price > 0 && coins < def.price) {
    shopBal.textContent = `Not enough 🪙 (you have ${coins}, need ${def.price})`; return;
  }
...
function tutShowStep(idx) {
  ...
  setTimeout(()=>{
    ...
    if (isInteractive) {
      ...
      const bx = (tutCube.x + 4) % 16, by = (tutCube.y + 4) % 16;
...
document.querySelectorAll('.pin-btn[data-v]').forEach(b=>{
  b.addEventListener('click',()=>{ if(pinBuffer.length<4){ pinBuffer+=b.dataset.v; updatePinDisplay(); } });
});
